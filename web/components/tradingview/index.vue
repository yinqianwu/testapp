...
import Datafeeds from './datafeeds/datafeeds.js'
  ...
  mounted() {
    ...
    this.socket.on('open', () => {
      this.socket.send({
        cmd: 'req',
        args: [
          'candle.' + this.interval + '.' + this.symbol.toLowerCase(),
          300,
          parseInt(Date.now() / 1000)
        ]
      })
    })
    console.log('loading data by websocket')
  ...
  methods: {
    ...
    onMessage(data) {
      ...
      if (data.name === 'kline') {
        ...
        const tickerstate = ticker + 'state'
        const tickerCallback = ticker + 'Callback'
        const onLoadedCallback = thats.cacheData[tickerCallback]
        ...
        if (thats.cacheData[ticker].length > 0) {
          thats.lastTime =
            thats.cacheData[ticker][thats.cacheData[ticker].length - 1].time
        }
      }

      if (data.name === 'kline_real') {
        const result = data.data.kline
        ...
        if (
          barsData.time > this.lastTime &&
          this.cacheData[ticker] &&
          this.cacheData[ticker].length
        ) {
          // 增量更新的数据直接加入缓存数组
    ...
    initMessage(
      ...
      // 获取需要请求的数据数目
      const limit = that.initLimit(resolution, rangeStartDate, rangeEndDate)
      // 商品名
      const symbol = that.symbol
      ...
      this.socket.send({
        cmd: 'req',
        args: [
          'candle.' + this.interval + '.' + symbol.toLowerCase(),
          limit,
          rangeEndDate
        ]
